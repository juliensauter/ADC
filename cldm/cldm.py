...
class ControlLDM(LatentDiffusion):
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        # C4: fix seed for log_images sampling so the same masks/noise are
        # used across log calls -> visual progress is comparable across steps.
        # Save & restore RNG state so training_step's RNG is not polluted.
        log_images_seed = int(os.environ.get("ADC_LOG_IMAGES_SEED", "42"))
        ...
        try:
            ...
            mps_rng_state = None
        torch.manual_seed(log_images_seed)
        if torch.cuda.is_available():
            torch.cuda.manual_seed_all(log_images_seed)
